...
void separate_points(t_point* points,
    int num_point,
    t_point from,
    t_point to,
    t_point* s1,
...
t_line*
convex_hull(t_point* points, int num_point, int* num_line)
{
    int capacity = 10;
    ...
    *num_line = 0;
    ...
    // 점들을 분리
    separate_points(
        points, num_point, points[0], points[num_point - 1], s1, s2, &n1, &n2);

    // upper hull을 구한다.
    lines = upper_hull(s1, n1, points[0], points[num_point - 1], lines,
        num_line, &capacity);
    lines = upper_hull(s2, n2, points[num_point - 1],
        points[0], lines, num_line, &capacity);
...
BOOL isUpper(int iA, int iB, int iC, t_point tpPoint)
{
    int result = iA * tpPoint.x + iB * tpPoint.y - iC;
    ...
    int* n2)
{
    int iA = to.y - from.y;
    int iB = from.x - to.x;
    int iC = (from.x * to.y) - (from.y * to.x);

    *n1 = 0;
    *n2 = 0;
    s1 = malloc(sizeof(t_point) * 0);
    s2 = malloc(sizeof(t_point) * 0);

    for (int i = 0; i < num_point; ++i) {
        BOOL isUp = isUpper(iA, iB, iC, points[i]);
        if (isUp == TRUE) {
            *n1++;
            s1 = realloc(s1, sizeof(t_point) * (*n1));
            s1[(*n1) - 1] = points[i];
        } else if (isUp == FALSE) {
            *n2++;
            s2 = realloc(s2, sizeof(t_point) * (*n1));
            s2[(*n2) - 1] = points[i];
        }
    }
}
